...
int v4l2_overlay_s_fbuf(int fd, int rotation)
{
    ...
    ret = v4l2_overlay_ioctl(fd, VIDIOC_G_FBUF, &fbuf, "VIDIOC_G_FBUF");
    if (ret)
        return ret;
    ...
    ret = v4l2_overlay_ioctl(fd, VIDIOC_S_FBUF, &fbuf, "VIDIOC_S_FBUF");
    if (ret)
        return ret;
...
int v4l2_overlay_query_buffer(int fd, int index, struct v4l2_buffer *buf)
{
    ...
    return v4l2_overlay_ioctl(fd, VIDIOC_QUERYBUF, buf, "VIDIOC_QUERYBUF");
}
...
{
    return v4l2_overlay_ioctl(fd, VIDIOC_QUERYCAP, caps, "VIDIOC_QUERYCAP");
}
...
int v4l2_overlay_stream_on(int fd)
{
    ...
    ret = v4l2_overlay_ioctl(fd, VIDIOC_STREAMON, &type, "VIDIOC_STREAMON");
...
int v4l2_overlay_stream_off(int fd)
{
    ...
    ret = v4l2_overlay_ioctl(fd, VIDIOC_STREAMOFF, &type, "VIDIOC_STREAMOFF");
...
int v4l2_overlay_q_buf(int fd, int buffer, int zerocopy)
{
    ...
    return v4l2_overlay_ioctl(fd, VIDIOC_QBUF, &buf, "VIDIOC_QBUF");
}
...
int v4l2_overlay_dq_buf(int fd, int *index, int zerocopy)
{
    ...
    ret = v4l2_overlay_ioctl(fd, VIDIOC_DQBUF, &buf, "VIDIOC_DQBUF");
    if (ret)
        return ret;
    *index = buf.index;
    return 0;
