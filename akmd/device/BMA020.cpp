...
: index(0), accelerometer(3600)
{
    ...
    SUCCEED(ioctl(fd, BMA150_SET_RANGE,     &param) == 0);
    
    param = BMA020_BW_50HZ;
    SUCCEED(ioctl(fd, BMA150_SET_BANDWIDTH, &param) == 0);
}
...
void BMA020::measure()
{
    ...
    SUCCEED(ioctl(fd, BMA150_READ_ACCEL_XYZ, &accels) == 0);
...
void BMA020::start()
{
    ...
    SUCCEED(ioctl(fd, BMA150_SET_MODE, &bmode) == 0);
}
